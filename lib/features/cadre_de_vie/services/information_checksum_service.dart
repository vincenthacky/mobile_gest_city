...
class InformationChecksumService {
  /// Calcule le checksum d'une information individuelle à partir de son contenu JSON
  ...
  /// Calcule le checksum d'une information à partir du InformationModel
  static String calculateInformationChecksum(Map<String, dynamic> informationJson) {
    // Créer une copie pour éviter de modifier l'original
    final Map<String, dynamic> cleanInformation = Map<String, dynamic>.from(informationJson);
    
    // Supprimer les champs qui ne doivent pas affecter le checksum
    cleanInformation.remove('created_at');
    cleanInformation.remove('updated_at');
    
    // Trier les clés pour assurer un ordre constant
    final sortedKeys = cleanInformation.keys.toList()..sort();
    final Map<String, dynamic> sortedInformation = {};
    for (final key in sortedKeys) {
      sortedInformation[key] = cleanInformation[key];
    }
    
    return calculateItemChecksum(sortedInformation);
  }
