...
class CotisationDetailPage extends StatelessWidget {
                    ...
                    child: Column(
                      children: [
                        ...
                        Container(
                          ...
                          child: InkWell(
                            onTap: () => _generateAndSharePdf(context, title, montant.toStringAsFixed(0), date, paymentMethod, cotisationId, isPaid ? 'Validé' : 'En attente'),
                            borderRadius: BorderRadius.circular(12),
                            child: Container(
                              padding: EdgeInsets.symmetric(
  ...
  // Générer et partager le PDF directement avec la fenêtre native
  Future<void> _generateAndSharePdf(BuildContext context, String title, String amount, String date, String paymentMethod, String paymentId, String status) async {
    ...
    // Afficher un indicateur de chargement avec possibilité d'annulation
    showDialog(
      context: context,
      barrierDismissible: true, // Permettre d'annuler en cliquant en dehors
    ...
    try {
      // Générer et partager le PDF avec timeout
      await PdfService.generateAndShareReceipt(
        ...
        onTimeout: () {
          throw Exception('Timeout - La génération du PDF a pris trop de temps');
        },
      );

      // Fermer le dialogue de chargement si toujours ouvert
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            ...
            action: SnackBarAction(
              ...
              onPressed: () => _generateAndSharePdf(context, title, amount, date, paymentMethod, paymentId, status),
            ),
          ),
        );
      }
